...
export default function SearchPage() {
  ...
  useEffect(() => {
    fetch("/api/prices?limit=5")
      .then((r) => r.json())
      .then((data) => { if (Array.isArray(data)) setRecentEntries(data); })
      .catch(() => {});
  }, []);
          ...
          {recentEntries.length === 0 ? (
            ...
            recentEntries.map((entry) => (
              <Link key={entry.id} href={`/item/${entry.itemId}`}
                className="block bg-white rounded-xl border border-gray-200 p-4 shadow-sm flex justify-between items-center">
                <div>
                ...
                <p className="text-lg font-bold text-brand-600">${entry.unitPrice.toFixed(2)}</p>
              </Link>
            ))
          )}
        </div>
